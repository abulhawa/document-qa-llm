...
def ingest_one(container_path: str) -> Dict:
    # 1) Read bytes (from /host-c or /staging etc.)
    data = load_document_bytes(container_path)
    ...
    mime = sniff_mime(container_path, data)
    chunks = split_into_chunks(container_path, data, mime)  # [{text, meta}, ...]
