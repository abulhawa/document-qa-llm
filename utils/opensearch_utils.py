    ...
    for i in range(0, len(unique), CHUNK):
        ...
        try:
            resp = client.delete_by_query(
                ...
                body={
                    "query": {
                        "terms": {"checksum": batch}
                    }
                },
                params={"refresh": "true", "conflicts": "proceed", "timeout": OPENSEARCH_REQUEST_TIMEOUT},
            )
            deleted = int(resp.get("deleted", 0))
            total_deleted += deleted
            logger.info(f"🗑️ OpenSearch deleted {deleted} docs for {len(batch)} checksum(s).")
        except exceptions.OpenSearchException as e:
            logger.exception(f"OpenSearch delete failed for a batch of {len(batch)} checksum(s): {e}")
...
def get_duplicate_checksums(limit: int = 10000) -> List[str]:
    ...
    try:
        ...
        dups = [b["key"] for b in buckets if b.get("distinct_paths", {}).get("value", 0) > 1]
        return dups
    except Exception as e:
            ...
            body = {
                ...
                "aggs": {
                    "by_checksum": {
                        "terms": {"field": "checksum", "size": limit},
                        "aggs": {
                            "distinct_paths": {"cardinality": {"field": "path.keyword"}},
                        },
                    }
                },
            }
