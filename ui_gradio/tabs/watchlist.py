...
def build_watchlist_tab() -> None:
    ...
    def _load_watchlist() -> tuple[list[str], gr.update, pd.DataFrame, str]:
        prefixes = watchlist_usecase.load_watchlist_prefixes()
        message = "" if prefixes else "No tracked folders yet. Add one above."
        return (
            prefixes,
            gr.update(choices=prefixes, value=prefixes[0] if prefixes else None),
    ...
    def _add_prefix(
    ...
    ) -> tuple[list[str], gr.update, pd.DataFrame, str]:
        cleaned = (prefix or "").strip()
        if not cleaned:
            return prefixes, gr.update(), _build_status_frame(prefixes), "Enter a valid path prefix."
        if cleaned in prefixes:
            return prefixes, gr.update(value=cleaned), _build_status_frame(prefixes), "Already tracked."
        ...
        return prefixes, gr.update(), _build_status_frame(prefixes), "Could not add prefix."

    def _refresh_all(prefixes: list[str]) -> tuple[pd.DataFrame, str]:
        if not prefixes:
            return _build_status_frame(prefixes), "No tracked folders to refresh."
        ...
        return _build_status_frame(prefixes), message

    def _refresh_status(prefix: str, prefixes: list[str]) -> tuple[pd.DataFrame, str]:
        if not prefix:
            return _build_status_frame(prefixes), "Select a folder to refresh."
    ...
    def _scan_disk(prefix: str, prefixes: list[str]) -> tuple[pd.DataFrame, str]:
        if not prefix:
            return _build_status_frame(prefixes), "Select a folder to scan."
    ...
    def _compute_remaining(prefix: str, prefixes: list[str]) -> tuple[pd.DataFrame, str]:
        if not prefix:
            return _build_status_frame(prefixes), "Select a folder to check."
    ...
    def _import_known(prefix: str, prefixes: list[str]) -> tuple[pd.DataFrame, str]:
        if not prefix:
            return _build_status_frame(prefixes), "Select a folder to import."
    ...
    def _import_chunks(prefix: str, prefixes: list[str]) -> tuple[pd.DataFrame, str]:
        if not prefix:
            return _build_status_frame(prefixes), "Select a folder to import."
    ...
    def _sync_indices(prefix: str, prefixes: list[str]) -> tuple[pd.DataFrame, str]:
        if not prefix:
            return _build_status_frame(prefixes), "Select a folder to sync."
    ...
    ) -> tuple[pd.DataFrame, list[dict[str, Any]], pd.DataFrame, str]:
        if not prefix:
            return (
                _build_status_frame(prefixes),
                records,
    ...
    def _remove_prefix(
        ...
        if not prefix:
            return prefixes, gr.update(), _build_status_frame(prefixes), "Select a folder to remove."
        ...
        return prefixes, gr.update(), _build_status_frame(prefixes), "Failed to remove. Try again."
    ...
    reload_button.click(
        _load_watchlist,
        outputs=[watchlist_state, prefix_select, status_table, action_status],
    )
    add_button.click(
        _add_prefix,
    ...
    refresh_all_button.click(
        ...
        outputs=[status_table, action_status],
    )
    ...
    reingest_button.click(
        ...
        outputs=[status_table, task_state, task_table, action_status],
    )
    remove_button.click(
        _remove_prefix,
        inputs=[prefix_select, watchlist_state],
