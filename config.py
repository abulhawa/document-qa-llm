...
_default_rerank_api_url = EMBEDDING_API_URL
if _default_rerank_api_url.endswith("/embed"):
    _default_rerank_api_url = _default_rerank_api_url[: -len("/embed")] + "/rerank"
RERANK_API_URL = _env_str("RERANK_API_URL", _default_rerank_api_url)
RETRIEVAL_ENABLE_RERANK = _env_bool("RETRIEVAL_ENABLE_RERANK", False)
RETRIEVAL_RERANK_TOP_N = _env_int("RETRIEVAL_RERANK_TOP_N", 5)
RETRIEVAL_RERANK_CANDIDATE_POOL = _env_int("RETRIEVAL_RERANK_CANDIDATE_POOL", 20)
RETRIEVAL_RERANK_TIMEOUT_CONNECT = _env_float("RETRIEVAL_RERANK_TIMEOUT_CONNECT", 2.0)
RETRIEVAL_RERANK_TIMEOUT_READ = _env_float("RETRIEVAL_RERANK_TIMEOUT_READ", 30.0)

# QA handoff policy (retrieval -> prompt context packing)
QA_HANDOFF_POLICY = _env_str("QA_HANDOFF_POLICY", "dynamic").strip().lower()
QA_HANDOFF_FIXED_TOP_K = _env_int("QA_HANDOFF_FIXED_TOP_K", 5)
QA_HANDOFF_DYNAMIC_RETRIEVAL_TOP_K = _env_int("QA_HANDOFF_DYNAMIC_RETRIEVAL_TOP_K", 7)
QA_HANDOFF_DYNAMIC_TOKEN_BUDGET = _env_int("QA_HANDOFF_DYNAMIC_TOKEN_BUDGET", 1200)
QA_HANDOFF_DYNAMIC_MIN_CHUNKS = _env_int("QA_HANDOFF_DYNAMIC_MIN_CHUNKS", 3)
QA_HANDOFF_DYNAMIC_MAX_CHUNKS = _env_int("QA_HANDOFF_DYNAMIC_MAX_CHUNKS", 0)
...
FINANCIAL_RECORDS_INDEX_BASE = _env_str(
    "FINANCIAL_RECORDS_INDEX_BASE", "financial_records"
)
FINANCIAL_RECORDS_INDEX = _namespaced(FINANCIAL_RECORDS_INDEX_BASE)
...
LLM_CACHE_STORE_PROMPT_TEXT = _env_bool("LLM_CACHE_STORE_PROMPT_TEXT", True)
...
QA_ENABLE_QUERY_PLANNING = _env_bool("QA_ENABLE_QUERY_PLANNING", False)
QA_ENABLE_HYDE = _env_bool("QA_ENABLE_HYDE", False)
...
def dump_config_for_debug() -> None:
    safe = {
        ...
        "FINANCIAL_RECORDS_INDEX": FINANCIAL_RECORDS_INDEX,
        ...
        "QA_ENABLE_QUERY_PLANNING": QA_ENABLE_QUERY_PLANNING,
        "QA_ENABLE_HYDE": QA_ENABLE_HYDE,
        "RETRIEVAL_ENABLE_RERANK": RETRIEVAL_ENABLE_RERANK,
        "RETRIEVAL_RERANK_TOP_N": RETRIEVAL_RERANK_TOP_N,
        "RETRIEVAL_RERANK_CANDIDATE_POOL": RETRIEVAL_RERANK_CANDIDATE_POOL,
        "RERANK_API_URL": RERANK_API_URL,
        "QA_HANDOFF_POLICY": QA_HANDOFF_POLICY,
        "QA_HANDOFF_FIXED_TOP_K": QA_HANDOFF_FIXED_TOP_K,
        "QA_HANDOFF_DYNAMIC_RETRIEVAL_TOP_K": QA_HANDOFF_DYNAMIC_RETRIEVAL_TOP_K,
        "QA_HANDOFF_DYNAMIC_TOKEN_BUDGET": QA_HANDOFF_DYNAMIC_TOKEN_BUDGET,
        "QA_HANDOFF_DYNAMIC_MIN_CHUNKS": QA_HANDOFF_DYNAMIC_MIN_CHUNKS,
        "QA_HANDOFF_DYNAMIC_MAX_CHUNKS": QA_HANDOFF_DYNAMIC_MAX_CHUNKS,
