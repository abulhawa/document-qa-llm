...
# Expected session_state keys: "topic_naming_rows" (from pages/11_topic_discovery.py)

st.set_page_config(page_title="Topic Naming Review", layout="wide")
...
if not rows_state:
    st.info("Run Topic Discovery → Generate names first.")
    st.stop()

rows = normalize_rows(rows_state)
...
df["needs_review"] = df.apply(
    lambda row: _needs_review(row, mixedness_threshold, confidence_threshold),
    axis=1,
)
...
if "mixedness" in filtered_df:
    mixedness_mask = _range_filter(
        filtered_df["mixedness"].fillna(-1.0),
        mixedness_range[0],
        mixedness_range[1],
    )
    if mixedness_range != (0.0, 1.0):
        mixedness_mask &= filtered_df["mixedness"].notna()
    filtered_df = filtered_df[mixedness_mask]

if "confidence" in filtered_df:
    confidence_mask = _range_filter(
        filtered_df["confidence"].fillna(-1.0),
        confidence_range[0],
        confidence_range[1],
    )
    if confidence_range != (0.0, 1.0):
        confidence_mask &= filtered_df["confidence"].notna()
...
if search_query:
    ...
    search_mask = (
        filtered_df["proposed_name"].fillna("").str.lower().str.contains(query)
    )
    example_paths = filtered_df["example_paths"].apply(
        lambda paths: " ".join(paths).lower()
    )
...
llm_used_pct = (
    metric_source["llm_used"].mean() * 100 if total_items else 0.0
)
cache_hit_pct = (
    metric_source["cache_hit"].mean() * 100 if total_items else 0.0
)
baseline_pct = (
    (
        (
...
high_mixedness_pct = (
    (
        metric_source["mixedness"].fillna(-1.0) >= mixedness_threshold
    ).mean()
    * 100
    if total_items
    else 0.0
...
median_confidence = (
    metric_source["confidence"].median() if total_items else None
)
p10_confidence = (
    metric_source["confidence"].quantile(0.1) if total_items else None
)
...
select_labels = (
    filtered_df.apply(
        lambda row: f"{row['level']} {row['id']} — {row['proposed_name']}", axis=1
    )
    .tolist()
)
selection_lookup = dict(zip(select_labels, filtered_df["row_key"].tolist(), strict=False))
...
selected_row = filtered_df.loc[filtered_df["row_key"] == selected_key].iloc[0]

st.divider()

st.subheader(f"{selected_row['level']} {selected_row['id']}: {selected_row['proposed_name']}")
